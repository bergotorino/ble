...
func NewSocket(id int) (*Socket, error) {
	...
	log.Println("set io cap")
	s.WriteCmd(0x0016, 1, []byte{0x3})
	rsp, err = s.ReadRsp()
	log.Println(rsp, err)
...
func (s *Socket) Pair(mac []byte, addrType byte) error {
	...
	/*
		...
		Possible values for the IO_Capability parameter:
			...
			4	KeyboardDisplay
	*/
	log.Printf("pairing mac %v, type %v", hex.EncodeToString(mac), addrType)

	b := []byte{}
	b = append(mac, addrType, 0x03) //io cap 0x3

	s.WriteCmd(0x0019, 1, b)
	_, err := s.ReadRsp()
	return err
